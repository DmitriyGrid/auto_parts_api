...
class PartService:
    ...
    async def get_parts(self, page: int = 1, limit: int = 10):
    ...
    async def get_part_by_id(self, part_id: int):
        query = select(AutoPart).where(AutoPart.id == part_id, AutoPart.deleted_at == None)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def create_part(self, part_data: PartCreate):
        new_part = AutoPart(**part_data.model_dump())
